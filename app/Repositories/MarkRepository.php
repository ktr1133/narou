...
class MarkRepository extends Repository
{
     ...
     * 
     * @param Mark $mark
     * @param CommonService $common_service 共通パーツサービス
     * @param Ma $ma
     */
     ...
     * @return Collection
     */
    public function getWRankingResult(Request $request): Collection
    {
        $ma = new Ma();
        $query = $ma->query();
    ...
    public function createMarkWRank(Request $request): Collection
    {
        ...
        $query = $this->ma->query();
        // 期間
        $time_span_column_point = $this->common_service->getTimeSpanColumn($request);
